...
For comparison, I input the original data to the Scikit-learn random forest regressor. Random forest classifiers train an ensemble of decision trees on random subsets of the data and features, then average the predictions across these trees. This approach can be adapted to continuous regression and can handle nonlinear relationships as well as correlated features, although it may have trouble extrapolating beyond the training data range. 

### Performance evaluation
The random forest regressor, which required pretty low effort and time, performed surprisingly well, achieving a MAE of 59 (with most bikes rented data ranging from 0 to 4000) and R<sup>2</sup> of 0.97 (this metric does not necessarily have the same implications for GLM as for OLS, but is useful for comparing performance e). However, with access to so many free parameters, this could be overfitting. When I split out a 20% testing data set, the performance of the random forest dropped to 0.85. Increasing the number of estimators to the default 100 improved this to 0.87, and increasing to 400 produced no improvement. Further improvement to 0.877 was achieved with coarse random search hyperparameter tuning. "Temperature" and "hour" had the highest feature importance, but this single measure of feature importance may also obscure the nature of its predictive value. For instance, although "rainfall" was a clear predictor of bike demand, it was ranked as sixth most predictive. I wonder how a feature that is highly predictive but is only relevant in a small subset of data points would be ranked.

The custom regression performed similarly on the training and test data sets at an R<sup>2</sup> of 0.89, took longer, and was more subjective. However, it offered more insight and natural interpretability. It could likely be improved by further refinement as well.
